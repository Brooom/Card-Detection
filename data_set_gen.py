...
images = [cv2.imread(file,0) for file in glob.glob(path+"/*.png")] #[:4]  
images = np.array(images)

back_images = [cv2.imread(file,0) for file in glob.glob("background/*.jpg")] #[7:]
            ...
            for i_pov in range(POV_data_size):
                ...
                for back in back_images:
                    for i_pos in range(pos_data_size):
                        ...
                        #save image
                        #print(set_path+'img_'+str(index)+"_"+str(count)+".png")
                        cv2.imwrite(set_path+'img_'+str(index).rjust(2,"0")+"_"+str(count).rjust(6,"0")+".png",img_pos)
